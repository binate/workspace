...
- **Proposing the deferral in your OWN message and treating a general "go ahead" as ratifying it — then later calling it "agreed."** This has bitten: I proposed "fix the leak now; the optimization un-revert would be a separate follow-up," the user said "go ahead" (authorizing the *fix*), and I then landed only the fix and described deferring the un-revert as "deferred as agreed." The user never agreed to defer it — "go ahead" ratified the work, not the carve-out, and the un-revert was the whole point of fixing the leak. A "go ahead"/"yes"/"do it" answer authorizes the WORK; it does NOT ratify a scope reduction you smuggled into the same proposal. If a follow-up is really separable, ask about THAT explicitly ("do you want the un-revert now or later?") and get a direct answer before treating it as deferred — and never write "as agreed" for a decision the user didn't actually make.
...
- **Batch, don't atomize.** A subagent should handle a *chunk* of work, not a single item. One subagent per file across 1000 files is NOT reasonable — batch so each subagent handles a sensible chunk (e.g. 10–20 files, or whatever divides the work into a manageable number of agents). Size the chunk so the total agent count stays small, not so that it balloons with the input.
- **Cap concurrency at ~10.** Generally launch **no more than 10 subagents at a time**, and even 10 only for genuinely large tasks. Prefer fewer. Under exceptional circumstances more may be warranted, but you **MUST get user confirmation first** before launching more than 10 concurrently — don't do it unilaterally.
- These two levers compose: batching keeps the *total* number of agents down; the concurrency cap keeps the number *in flight* down. Apply both.
...
### CI Green Is Not a Goal in Itself

CI being green is not a goal in itself. The goal of CI is to **accurately and
completely reflect code correctness** (while doing so efficiently/quickly/etc.).
If a test is passing but not actually testing what's intended, fix the test —
even if it then fails and turns CI red. Fixing a test (or test harness, etc.)
does not change the underlying code correctness; it improves the
accuracy/completeness of the CI results. A red CI that faithfully reports a real
defect is strictly better than a green CI that hides one.
...
This has bitten badly (native-backend within-block reload-caching): a hang was attributed to a "mid-instruction clobber-invalidation" gap and chased through ~10 rebuild cycles of escalating machinery (fixup-count generations, threading the assembler through ~45 call sites). The actual bug was unrelated and obvious the moment a working-vs-broken disassembly of the SAME function was diffed: a loop increment stored the PRE-increment value (`str x9, [slot]` where x9 still held the old `i`) — because `AssignReg` appended a duplicate `id->reg` entry and `LookupReg` returned the stale first one. The one-line fix (AssignReg replaces instead of appends) made the whole generation/threading edifice unnecessary, and it was all deleted. Tells you are in this trap: you are on your Nth rebuild testing a *mechanism theory* and still have not looked at the actual bad instructions; each "fix" changes the symptom (hang→abort→hang) instead of resolving it. STOP and disassemble — build a known-good variant (e.g. revert just the suspect line), disassemble the same function from both, and read what actually differs at the miscompile site. On a stripped/self-linked binary, `lldb -b -s <cmds>` disassembling a fixed address range works when `otool`/`nm` show no symbols; `sample <pid>` locates a hang's hot loop by file offset.
...
**Before deep-diagnosing a "pre-existing" failure/hang, confirm it still reproduces on CURRENT main — rebase/resync your worktree first.** Your worktree base can be many commits behind local `main` (concurrent workers land constantly), and a failure you observe may already be fixed upstream. Reproducing on a stale base then investigating is wasted effort. This has bitten: a `pkg/binate/repl` unit-test infinite loop was reproduced ~a dozen times and diagnosed at length on a base ~20 commits stale — it was a `bnc` native-codegen miscompile (the VM ran repl fine; only the compiled `test_bin` spun) that a concurrent codegen commit had **already fixed**; rebasing onto current `main` made it vanish (both interpreted and compiled repl suites green). Cheap defense: `git -C <worktree> fetch <main-checkout> main && git -C <worktree> rebase FETCH_HEAD`, then re-run the failing check, BEFORE sinking time into root-causing. If it still reproduces post-rebase, it is genuinely open; if it disappears, `git log <old-base>..main -- <relevant-pkgs>` finds the fix.
...
The Go bootstrap interpreter has been retired. The operative constraint for cmd/bnc's tree is no longer "the bootstrap subset" (that term was, by definition, what the Go interp supported); it's **"must be compilable by the current BUILDER bnc"**. Do NOT hardcode a version number here — it goes stale. The exact pinned BUILDER is in the tree's **`BUILDER_VERSION`** file; consult that (and the corresponding tagged `bnc` revision, if you need to know precisely what a given release supports) to determine the constraint. The BUILDER bnc is built from a source snapshot, so its accepted language tracks the tree closely and the gap keeps shrinking.

The BUILDER-compiled surface is **`cmd/bnc` and its dependency tree (and their tests)** — nothing else. Everything else (bni, bnlint, the bytecode VM, the runtime, the linter library) is built by `bnc` first and then exercised. Anything in `cmd/bnc`'s transitive imports — pkg/{ast,bootstrap,buf,builtin/testing,check,codegen,debug,ir,irbuild,irdata,irgen,iropcode,iropt,irsym,irutil,lexer,loader,mangle,native,native/arm64,native/common,parser,token,types,asm,asm/aarch64,asm/arm32,asm/x64,asm/elf,asm/macho,asm/parse,asm/assemble,link,optflags,sha256} — must stay BUILDER-compilable.  (`check` is the type checker, split out of `types` (which is now the pure Type data model — layout/ABI/identity/naming); `check` imports `types`, never the reverse.  `irbuild` is the IR emit/construction API + verifier and `iropt` the optimizer, both split out of `ir` (now the pure IR data model); `irgen` is the IR-gen driver, `irsym`/`irutil` its symbol/util leaves.  `iropcode` is the IR opcode enum + `OpName`, extracted from `ir` as a leaf package; pure consts + one switch.  `irdata` is the backend-neutral static-data manifest — `DataGlobal`/`DataTerm` + the `Build*` record builders — also extracted from `ir` as a `@Module`-free leaf; the `Collect*` passes that need `@Module` stay in `ir`, which imports `irdata`.) cmd/bnc embeds the assembler to assemble its `.s` runtime files in-process (`pkg/binate/asm/assemble`, which dispatches parse → the arm32/x64/aarch64 encoders → the elf/macho writers), so the WHOLE asm text-assembler stack — `asm/parse`, every arch subpackage, and both object writers — is now in the BUILDER-compiled surface, not merely reachable-in-future.  (`cmd/bnas` is still built by `bnc` first as a standalone CLI tool, but its core is this same BUILDER-compiled `asm/assemble` library.)  `ir` also depends on `pkg/stdx/containers/{vec,iter}` (`Module.FuncSigs` is a `vec.Vec` for amortized growth), so those stdlib containers are in the BUILDER-compiled surface too — proven BUILDER-clean by the gen1 build.  cmd/bnc also links programs in-process via the self-hosted linker (`pkg/binate/link`, reached by `--linker bnld`), so `link` and its `pkg/binate/sha256` dep (the Mach-O ad-hoc code-signer's SHA-256) are in the BUILDER-compiled surface too — the linker library must now stay BUILDER-compilable (proven by the gen1 build).  (`link` also imports `asm/*` and `buf`/`os`, already listed above.)  `optflags` registers bnc's (and bni's) `-f<pass>` / `-fno-<pass>` switches on a `pkg/std/flags` FlagSet.

All BUILDER-compilable code must:

1. **Stay within what the pinned BUILDER accepts.** At this point there are very few *language* constraints — the BUILDER's accepted language is close to the full language (interfaces, generics, function values, etc. all work in the BUILDER-compiled tree today), and the gap keeps shrinking. So do NOT work from a hardcoded feature blocklist (it goes stale). When in doubt whether the pinned BUILDER supports a construct, **test it directly** against the BUILDER (`scripts/fetch-builder.sh --tool bnc`) per the note below. The evolution from here is mostly *capability*, not language: expanding the standard library (sometimes via `stdx` → `std` promotion cycles) and adding tooling (e.g. our own linker, `bnld`). So the practical BUILDER question is usually "does the pinned BUILDER's tree/stdlib already have what I'm importing?" — which the gen1 build answers. (`explorations/bootstrap-subset.md` is a historically-framed list, useful for context but not authoritative on the current BUILDER.)
...
For code that is *not* BUILDER-compilable (because it lives outside bnc's tree), you may use the full language freely. Code inside bnc's tree (and its tests) must stick to what the pinned BUILDER accepts — but per item 1, that gap is now small, so this rarely bites at the language level.
